...
void createFile()
{
    ...
    for (int i = 0; i < 5; i++)
    {
        ...
        t1 = steady_clock::now();
        vector<Students> vargS, kietS;
        for(Students t : tempStud)
            if(t.vid<5)
            vargS.push_back(t);           
        ...
        t2 = steady_clock::now();
        time_span = duration_cast<duration<double>>(t2 - t1);
        cout<<"studentu padalinimas i 2 vec uztruko: "<<time_span.count()<<endl;
        t1 = steady_clock::now();
        for (Students k : kietS)
        {
